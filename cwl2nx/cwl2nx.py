...
# }
DEFAULT_DISPLAY_PARAMS: dict[NodeTypeStr, dict[str, str | float | int]] = {
    "WorkflowStep": {"color": "yellow"},
    "WorkflowStepInput": {"color": "blue"},
    "WorkflowStepOutput": {"color": "magenta"},
    "WorkflowInputParameter": {"color": "green"},
    "WorkflowOutputParameter": {"color": "red"},
}
...
class CWLToNetworkxConnector:
    ...
    def parse_and_validate_cwl(self) -> Workflow:
        ...
        try:
            ...
            parsed_cwl_version = cwl_version(raw_yaml)
            if parsed_cwl_version is None:
                raise Exception("Unrecognized cwl version")
            self.cwl_version = parsed_cwl_version
    ...
    def convert_to_networkx(self) -> nx.DiGraph:
        ...
        # connects WorkflowInputParameter to WorkflowStep
        # and creates WorkflowStepInput nodes
        ...
        # Connecting WorkflowOutputParameters to WorkflowStepOutput
        for each_output in self.cwl_utils_graph.outputs:
            if each_output.outputSource in self.nx_graph.nodes:
                self.nx_graph.add_edge(each_output.outputSource, each_output.id)
